    ...
    class Roadmap {
        ...
        adjustCenters() {
            ...
            .forEach(function(interval) {
                interval
                .forEach(function(dependencyChain) {
        ...
        buildRenderTree(section, anglePerEndPoint, featureData, parentFeatureId) {
            ...
            if (Array.isArray(featureData)) {
                ...
                angle = (360 / this.sections) * section +                       // start angle of section
        ...
        buildFeatureChains(interval, parentFeatureId, featureId) {
            ...
            if (this.featureChains[interval]) {
                this.featureChains[interval]
                .forEach(function(featureChain, index) {
        ...
        fineTuneLabelSpacing(angleInDegrees) {
            return  Math.abs(
                        ...
                        Math.sin(
                            ((angleInDegrees * 2) % 180) / 180 * Math.PI
                        )
                    );
        }
            ...
            .forEach(function(feature, index) {
                if (index > 0) {
                    ...
                    self.paper.g(
                        ...
                        .addClass(
                            'roadmap__feature-dot js_featureDot ' +
                            self.getColorOfSection(feature.section, 'stroke')
                        ),
                        ...
                        function() {
                            if (!this.hasClass(self.stateClass.filteredOut)) {
                                ...
                                .html(
                                    ...
                                    $('.js_legendEntry > h3:contains(' + feature.name + ')').parent('li').html()
                                )
                                .stop(true, true)
        ...
        renderDiagramLabel(text) {
            // draw circle
            this.paper
            .circle('50%', '50%', this.diagramRadii.segmentMin + '%')
            .addClass('roadmap__label');
            ...
            if ((text.match(/(\n)|(<br>)|(<br[\s]?\/>)/g) || []).length > 1) { // regex to find line breaks
        ...
        renderLegend() {
            ...
            this.data.roadmap
            .forEach(function(sectionData, sectionNumber) {
        ...
        getCartesianCenter(angle, radius, center) {
            ...
            return  this.polar2Cartesian(
                        center.x,
                        center.y,
                        this.perc2Abs(radius + '%'),
                        angle
                    );
        }
        ...
        describeArc(centerX, centerY, radius, startAngle, endAngle, noMoveTo = false) {
            var start       = this.polar2Cartesian(centerX, centerY, radius, endAngle),
                ...
                d           = [
                                ...
                                'A', radius, radius, rotation, largeArc, sweep, end.x, end.y    // draw arc
                              ];
        ...
        highlightSegments(trigger, self, type) {
            Snap.selectAll('.js_segment.js_' + type + '-' + trigger.attr('data-' + type))
            .forEach(function(e) {
        ...
        removeSegmentHighlights(self) {
            Snap.selectAll('.js_segment.' + self.stateClass.highlighted)
            .forEach(function(e) {
        ...
        applySectionFilter(trigger, self) {
            if (trigger.hasClass(self.stateClass.selected)) {
                ...
                Snap.selectAll('.js_segment.js_section-' + trigger.attr('data-section'))
                .forEach(function(e) {
                    e.addClass(self.stateClass.selected);
                });

                // filter out all features that don't belong to the selected section
                Snap.selectAll('.js_feature:not(.js_section-' + trigger.attr('data-section') + ')')
                .forEach(function(e) {
                ...
                $('.js_legendSection')
                .addClass(self.stateClass.filteredOut);

                $('#js_legendSection-' + trigger.attr('data-section'))
        ...
        applyIntervalFilter(trigger, self) {
            if (trigger.hasClass(self.stateClass.selected)) {
                ...
                Snap.selectAll('.js_segment.js_interval-' + trigger.attr('data-interval'))
                .forEach(function(e) {
                    e.addClass(self.stateClass.selected);
                });

                // filter out all features in the diagram that don't belong to the selected interval
                Snap.selectAll('.js_feature:not(.js_interval-' + trigger.attr('data-interval') + ')')
                .forEach(function(e) {
                    e.addClass(self.stateClass.filteredOut);
                });
                ...
                $('.js_legendEntry.js_interval-' + trigger.attr('data-interval'))
                .removeClass(self.stateClass.filteredOut);
            }
        }
